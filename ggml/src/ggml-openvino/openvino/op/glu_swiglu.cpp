...
OutputVector translate_glu_swiglu(const NodeContext & context) {
    ...
    if (context.get_input_size() == 2) {
        ...
        // Opt-S11: when the split is even (2*nc == last_dim), emit v1::Split
        // which intel_gpu maps to a crop primitive (view, no kernel dispatch),
        // eligible for in-place buffer fusing. v8::Slice goes through a full
        // copy kernel every time. Fallback to Slice only for the odd-dim case.
